...
**Medium (~50 words):**  
Vnansial is a free, Indonesian-language platform that helps everyday people avoid investment scams, predatory loans, and financial fraud. An AI agent powered by SumoPod (`qwen3.6-flash`) autonomously runs tools—OJK-style entity checks, loan calculators, fraud reporting guides, and optional social outreach via Repliz—so users get answers and actions, not generic chat.
...
We built for **OpenClaw Agenthon 2026** with a path toward a full **multi-agent orchestrator** (Investigator, Analyst, Guardian, Outreach). The **shipping MVP** uses a single agent with an **autonomous tool-calling loop** (up to 8 LLM↔tool rounds per request) on SumoPod’s OpenAI-compatible API. Optional **Repliz Gold** integration lets the agent list social accounts and schedule literacy posts—only with explicit user consent.
...
- **Asisten AI** — SumoPod agent with tools: investment check, loan calc, red flags, fraud guide, Repliz (optional)  
...
| Social (optional) | Repliz Public API — HTTP Basic Auth |
...
- **Repliz** — optional credentials; agent must not post without explicit user confirmation  
...
- **Repliz integration** for literacy outreach (when configured)  
...
`React` · `TypeScript` · `Vite` · `Tailwind CSS` · `Framer Motion` · `Node.js` · `Express` · `OpenAI SDK` · `SumoPod` · `qwen3.6-flash` · `Repliz API` · `Docker` · `Vitest` · `GitHub Actions`
...
| **Tool calling** | 7 functions: investment check, loan calc, red flags, fraud guide, Repliz list/schedule |
| **Agent loop** | Up to 8 autonomous LLM↔tool iterations per `/api/agent/chat` request |
| **Repliz** (optional) | Schedule financial literacy posts to connected social accounts |
...
| **Multi-agent** | ⏳ **Designed, not fully shipped:** Orchestrator + Investigator / Analyst / Guardian / Outreach documented in README & pitch; MVP = single agent with all tools |
| **Not a basic chatbot** | ✅ Invokes calculators, lookups, guides, Repliz APIs with logged `toolCalls` |
...
| 1:25–1:45 | (Optional) Repliz: list accounts / draft schedule — or skip if no creds | *"Opsional: jadwalkan edukasi ke media sosial via Repliz."* |
